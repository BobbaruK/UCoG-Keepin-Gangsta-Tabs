...
import z from "zod";
import { addGamblingFeature } from "../../actions/add";
...
const AddGamblingSizeForm = () => {
  ...
  const form = useForm<z.infer<typeof AddGamblingFeatureSchema>>({
    ...
    defaultValues: {
      ...
      cash_on_hand: 0,
      weekly_cost: 0,
      type: GamblingFeatureType.OCCASIONAL,
      is_dlc: false,
    },
  });
  ...
  const { formId, inputId } = formInputId(
    `add-${gamblingFeatureTitle.label.singular.toLowerCase()}-form`,
  );

  const onSubmit = (values: z.infer<typeof AddGamblingFeatureSchema>) => {
    startTransition(async () => {
      addGamblingFeature(values)
        .then(async (data) => {
          if (data.error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Add ${gamblingFeatureTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
  ...
  return (
    <div className={cn("flex flex-col gap-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[172px]" />
      </div>
    </div>
  );
}
