...
export const editGamblingFeature = async (
  id: string,
...
> => {
  ...
  const permissions = await auth.api.userHasPermission({
    body: {
      ...
      permission: { gambling_features: ["update"] },
    },
  });
  ...
  const { name, weekly_cost, cash_on_hand, type, is_dlc } =
    validatedFields.data;

  try {
    const gamblingFeature = await db.cog_gambling_feature.update({
      where: {
        id,
      },
      data: {
