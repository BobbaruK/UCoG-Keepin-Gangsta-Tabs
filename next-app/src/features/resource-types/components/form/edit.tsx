...
import { editResourceType } from "../../actions/edit";
...
const EditResourceTypeForm = ({ resourceType }: Props) => {
  ...
  const onSubmit = (values: z.infer<typeof AddResourceTypeSchema>) => {
    startTransition(async () => {
      editResourceType(resourceType.id, values)
        .then(async (data) => {
          if (data.error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Save ${resourceTypesTitle.label.singular.toLowerCase()}`}
          type="submit"
          className="h-9"
          disabled={isPending}
          skeletonClassName="h-9 w-[151px]"
          variant={"success"}
        />
      </div>
    </form>
...
export default EditResourceTypeForm;
