...
const PaginationActions = () => {
  const { isLoading, startTransition, dataSelected } =
    useTableContext<ResourceType>();
  const [{}, setSearchParams] = useSearchParams(startTransition);
  const { handleCopy } = useCustomCopyToClipboard();
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);
  const { data: session } = useSession();

  const resourceTypeIdBatches = chunkArray(dataSelected, BATCH_ITEMS);

  const handleDeleteLaws = () => {
    setOpenDeleteDialog(false);

    startTransition(async () => {
      for (const batch of resourceTypeIdBatches) {
        const results = (await Promise.allSettled(
          batch.map((resourceType) => deleteResourceType(resourceType.id)),
        )) as {
          status: string;
          value: {
            error?: string;
    ...
    <>
      <ResponsiveDialog
        ...
        header={{
          ...
          description: `This action cannot be undone. This will permanently delete selected ${resourceTypesTitle.label.singular.toLowerCase()}(s) and remove it's data from our servers.`,
        }}
      >
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start">
          <DropdownMenuItem
            onClick={handleCopy(
              dataSelected.map((resource) => resource.id).join("\n") || "",
            )}
          >
            <CopyIcon />
            Copy id(s)
          ...
          {session && session.user.role !== UserRole.USER && (
            <>
              ...
              >
                <TrashIcon />
                Delete {resourceTypesTitle.label.singular.toLowerCase()}(s)
              </DropdownMenuItem>
            </>
          )}
        </DropdownMenuContent>
