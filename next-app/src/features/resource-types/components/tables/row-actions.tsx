...
const RowActions = ({ resourceType }: Props) => {
  ...
  const handleCopy = (text: string) => () => {
    ...
    copy(text)
      .then(() => {
        toast.success(
          `Copied ${resourceTypesTitle.label.singular.toLowerCase()} ID.`,
          {
            description: text,
          },
        );
    ...
    <>
      <ResponsiveDialog
        ...
        header={{
          ...
          description: `This action cannot be undone. This will permanently delete this ${resourceTypesTitle.label.singular.toLowerCase()} and remove it's data from our servers.`,
        }}
      >
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          <DropdownMenuItem onClick={handleCopy(resourceType.id)}>
            <CopyIcon /> Copy ID
          </DropdownMenuItem>
          ...
          {session && session.user.role !== UserRole.USER && (
            <>
              ...
              <DropdownMenuItem asChild>
                <Link
                  href={`${resourceTypesTitle.href}/${resourceType.id}/edit`}
                >
                  <EditIcon />
                  Edit {resourceTypesTitle.label.singular.toLowerCase()}
                </Link>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem
              ...
              >
                <TrashIcon />
                Delete {resourceTypesTitle.label.singular.toLowerCase()}
              </DropdownMenuItem>
            </>
          )}
        </DropdownMenuContent>
