...
const PaginationActions = ({
  ...
  const nationalitiesData =
    dataSelected.type === "nationalities" && dataSelected.data
      ? dataSelected.data
      : [];
    ...
    startTransition(async () => {
      for (const batch of lawIdBatches) {
        const results = (await Promise.allSettled(
          batch.map((trait) => deleteNationality(trait.id)),
        )) as {
          status: string;
          value: {
            error?: string;
    ...
    <>
      <ResponsiveDialog
        ...
        header={{
          ...
          description: `This action cannot be undone. This will permanently delete selected ${nationalitiesTitle.label.singular.toLowerCase()}(s) and remove it's data from our servers.`,
        }}
      >
        ...
        <DropdownMenuContent align="start">
          ...
          {session && session.user.role !== UserRole.USER && (
            <>
              ...
              >
                <TrashIcon />
                Delete {nationalitiesTitle.label.singular.toLowerCase()}(s)
              </DropdownMenuItem>
            </>
          )}
        </DropdownMenuContent>
