...
export const editBuildingPassive = async (
  id: string,
...
> => {
  ...
  const permissions = await auth.api.userHasPermission({
    body: {
      ...
      permission: { building_passive_production: ["update"] },
    },
  });
  ...
  try {
    const buildingPassive = await db.cog_building_passive_production.update({
      where: {
        id,
      },
      data: {
