...
import z from "zod";
import { addBuildingPassive } from "../../actions/add";
...
const AddBuildingPassiveForm = ({ resources = [] }: Props) => {
  ...
  const { formId, inputId } = formInputId(
    `add-${buildingPassiveTitle.label.singular.toLowerCase()}-form`,
  );

  const onSubmit = (values: z.infer<typeof AddBuildingPassiveSchema>) => {
    startTransition(async () => {
      addBuildingPassive(values)
        .then(async (data) => {
          if (data.error) {
  ...
  return (
    ...
    >
                ...
                <Popover open={comboxResource} onOpenChange={setComboxResource}>
                  ...
                  <PopoverContent className="p-0" align="start">
                    <Command>
                      ...
                      <CommandList>
                        <CommandEmpty>
                          No {resourcesTitle.label.singular.toLowerCase()}{" "}
                          found.
                        </CommandEmpty>
                        <CommandGroup>
                          {resources
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Add ${buildingPassiveTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
          skeletonClassName="h-9 w-[238px]"
          variant={"success"}
        />
      </div>
    </form>
...
export default AddBuildingPassiveForm;

export function AddBuildingPassiveFormSkeleton({
  className,
  ...restProps
  ...
  return (
    <div className={cn("space-y-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[238px]" />
      </div>
    </div>
  );
}
