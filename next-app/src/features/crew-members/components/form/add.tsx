...
const AddCrewMemberForm = ({
  ...
  const { formId, inputId } = formInputId(
    `add-${crewMembersTitle.label.singular.toLowerCase()}-form`,
  );
      ...
      addCrewMember({
        ...
        .then(async (data) => {
          ...
          if (data.success) {
            ...
            // router.push(
            //   `${playthroughTitle.href}/${playthroughId + crewMembersTitle.href}/${data.crewMemberId}/edit/experience`,
            // );
          }
        })
        .catch(() => {
...
export function AddCrewMemberFormSkeleton({
  className,
  ...restProps
  ...
  return (
    <div className={cn("flex flex-col gap-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[149px]" />
      </div>
    </div>
  );
}
