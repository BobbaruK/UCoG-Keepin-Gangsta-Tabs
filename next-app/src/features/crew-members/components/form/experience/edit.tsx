...
import { editExperiences } from "@/features/crew-members/actions/experience/edit";
import { CrewMember } from "@/features/crew-members/types/crew-member";
...
const EditExperienceForm = ({ member, playthroughId, levels }: Props) => {
  const [isPending, startTransition] = useTransition();
  const router = useRouter();
  const form = useForm<z.infer<typeof experienceSchema>>({
    resolver: zodResolver(experienceSchema),
    defaultValues: {
      experiences: member.experience.map((experience) => ({
  ...
  function onSubmit(values: z.infer<typeof experienceSchema>) {
    startTransition(async () => {
      editExperiences({ values, memberId: member.id })
        .then(async (data) => {
          if (data.error) {
