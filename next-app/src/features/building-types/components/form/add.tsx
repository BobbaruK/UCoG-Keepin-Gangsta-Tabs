...
import z from "zod";
import { addBuildingType } from "../../actions/add";
import { AddBuildingTypeSchema } from "../../schemas/add-building-type";

const AddBuildingTypeForm = () => {
  ...
  const { formId, inputId } = formInputId(
    `add-${buildingTypesTitle.label.singular.toLowerCase()}-form`,
  );

  const onSubmit = (values: z.infer<typeof AddBuildingTypeSchema>) => {
    startTransition(async () => {
      addBuildingType(values)
        .then(async (data) => {
          if (data.error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Add ${buildingTypesTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
...
export function AddBuildingTypeFormSkeleton({
  className,
  ...restProps
  ...
  return (
    <div className={cn("space-y-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[147px]" />
      </div>
    </div>
  );
}
