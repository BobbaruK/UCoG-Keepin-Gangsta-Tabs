    ...
    startTransition(async () => {
      for (const batch of playthroughIdBatches) {
        const results = (await Promise.allSettled(
          batch.map((building) => deleteBuilding(building)),
        )) as {
          status: string;
          value: {
            error?: string;
    ...
    <>
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start">
          ...
          {session &&
            dataSelected[0]?.user_id === session.user.id &&
            !dataSelected[0]?.playthrough.is_finished && (
              <>
                ...
                >
                  <TrashIcon />
                  Delete {buildingTitle.label.singular.toLowerCase()}(s)
                </DropdownMenuItem>
              </>
            )}
        </DropdownMenuContent>
