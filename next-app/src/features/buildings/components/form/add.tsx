...
import z from "zod";
import { addBuilding } from "../../actions/add";
...
const AddBuildingForm = ({
  ...
  const { formId, inputId } = formInputId(
    `add-${buildingTitle.label.singular.toLowerCase()}-form`,
  );
      ...
      addBuilding({
        ...
        .then(async (data) => {
          ...
          if (data.success) {
            ...
            router.push(
              `${playthroughTitle.href}/${playthrough.id + buildingTitle.href}`,
            );
          }
        })
        .catch(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Add ${buildingTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
...
export function AddBuildingFormSkeleton({
  className,
  ...restProps
  ...
  return (
    <div className={cn("flex flex-col gap-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[115px]" />
      </div>
    </div>
  );
}
