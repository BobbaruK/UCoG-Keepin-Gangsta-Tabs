...
import { editBuildingPassiveDuration } from "../../actions/edit";
...
const EditBuildingPassiveDurationForm = ({ buildingPassiveDuration }: Props) => {
  ...
  const form = useForm<z.infer<typeof AddBuildingPassiveDurationSchema>>({
    ...
    defaultValues: {
      name: buildingPassiveDuration.name,
      turns: buildingPassiveDuration.turns,
    },
  });
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);

  const { formId, inputId } = formInputId(
    `edit-${buildingPassiveDurationTitle.label.singular.toLowerCase()}-form`,
  );
  ...
  const onSubmit = (
    ...
    startTransition(async () => {
      editBuildingPassiveDuration(buildingPassiveDuration.id, values)
        .then(async (data) => {
          if (data.error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Save ${buildingPassiveDurationTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
...
export default EditBuildingPassiveDurationForm;

export function EditBuildingPassiveDurationFormSkeleton({
  className,
  ...restProps
  ...
  return (
    <div className={cn("space-y-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[300px]" />
      </div>
    </div>
  );
}
