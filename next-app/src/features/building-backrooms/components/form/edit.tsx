...
import { editBuildingBackroom } from "../../actions/edit";
import { AddBuildingBackroomSchema } from "../../schemas/add-building-backroom";

interface Props {
  buildingBackroom: BuildingBackroom;
}

const EditBuildingBackroomForm = ({ buildingBackroom }: Props) => {
  const [isPending, startTransition] = useTransition();
  const router = useRouter();
  const form = useForm<z.infer<typeof AddBuildingBackroomSchema>>({
    resolver: zodResolver(AddBuildingBackroomSchema),
    defaultValues: {
      name: buildingBackroom.name,
    },
  });
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);

  const { formId, inputId } = formInputId(
    `edit-${buildingBackroomsTitle.label.singular.toLowerCase()}-form`,
  );

  const onSubmit = (values: z.infer<typeof AddBuildingBackroomSchema>) => {
    startTransition(async () => {
      editBuildingBackroom(buildingBackroom.id, values)
        .then(async (data) => {
          if (data.error) {
          ...
          if (data.success) {
            ...
            setTimeout(() => {
              ...
              router.push(buildingBackroomsTitle.href);
            }, 250);
          }
        })
        .catch(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Save ${buildingBackroomsTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
  ...
  return (
    <div className={cn("space-y-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[185px]" />
      </div>
    </div>
  );
}
