...
export const columns = ({
  ...
  visibleUsers: GamblingSize[];
}): ColumnDef<GamblingSize>[] => [
  // Select
  {
    ...columnId({ id: "select" }),
    meta: {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2">
        <CustomButton
          ...
          size={"sm"}
          linkHref={`${gamblingSizeTitle.href}/${row.original.id}`}
          variant={"link"}
          className=""
  ...
  // Features
  {
    ...columnId({ id: "max_features" }),
    meta: {
      label: "Features",
    },
    accessorFn: (originalRow) => originalRow.max_features,
    enableHiding: false,
    enableSorting: true,
    enablePinning: true,
    // size: 110,
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2 px-3">
        <Badge variant={"outline"}>{row.original.max_features}</Badge>
      </div>
    ),
  },
  // Is DLC
  {
    ...
    cell: ({ row }) => (
      <div className="flex items-center gap-2 px-3">
        <Badge variant={row.original.is_dlc ? "success" : "warning"}>
