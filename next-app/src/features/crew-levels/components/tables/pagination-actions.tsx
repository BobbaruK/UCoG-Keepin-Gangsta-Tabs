...
const PaginationActions = () => {
  const { isLoading, startTransition, dataSelected } =
    useTableContext<CrewLevel>();
  const [{}, setSearchParams] = useSearchParams(startTransition);
  const { handleCopy } = useCustomCopyToClipboard();
  const [openDeleteDialog, setOpenDeleteDialog] = useState(false);
  const { data: session } = useSession();

  const levelIdBatches = chunkArray(dataSelected, BATCH_ITEMS);

  const handleDeleteLaws = () => {
    setOpenDeleteDialog(false);

    startTransition(async () => {
      for (const batch of levelIdBatches) {
        const results = (await Promise.allSettled(
          batch.map((level) => deleteCrewLevel(level.id)),
        )) as {
          status: string;
          value: {
            error?: string;
    ...
    <>
      <ResponsiveDialog
        ...
        header={{
          ...
          description: `This action cannot be undone. This will permanently delete selected ${crewLevelsTitle.label.singular.toLowerCase()}(s) and remove it's data from our servers.`,
        }}
      >
        ...
        <DropdownMenuContent align="start">
          ...
          {session && session.user.role !== UserRole.USER && (
            <>
              ...
              >
                <TrashIcon />
                Delete {crewLevelsTitle.label.singular.toLowerCase()}(s)
              </DropdownMenuItem>
            </>
          )}
        </DropdownMenuContent>
