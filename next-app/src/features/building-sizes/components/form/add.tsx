...
import z from "zod";
import { addBuildingSize } from "../../actions/add";
import { AddBuildingSizeSchema } from "../../schemas/add-building-size";

const AddBuildingSizeForm = () => {
  ...
  const { formId, inputId } = formInputId(
    `add-${buildingSizesTitle.label.singular.toLowerCase()}-form`,
  );

  const onSubmit = (values: z.infer<typeof AddBuildingSizeSchema>) => {
    startTransition(async () => {
      addBuildingSize(values)
        .then(async (data) => {
          if (data.error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <CustomButton
          buttonLabel={`Add ${buildingSizesTitle.label.singular.toLowerCase()}`}
          type="submit"
          className=""
          disabled={isPending}
...
export default AddBuildingSizeForm;
  ...
  return (
    <div className={cn("space-y-7", className)} {...restProps}>
      ...
      <div className="flex flex-wrap items-center justify-end gap-4">
        ...
        <Skeleton className="bg-success h-9 w-[142px]" />
      </div>
    </div>
  );
}
